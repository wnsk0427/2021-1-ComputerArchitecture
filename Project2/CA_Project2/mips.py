...
how to Run : $ python3 ./mips.py <assembly file>
"""
...
class MIPS :
    def __init__(self):
        ...
        self.instr_i_format = ["addiu", "andi", "beq", "bne", "lui", "lw", "ori", "sltiu", "sw" , "lb", "sb"]
    ...
    def read_files(self, file_name):
        ...
        while True:
            ...
            if len(args) > 0 and args[0] == self.diretives[1] : # memory에 .word 할당
                self.data_section_lines += 1
                self.memory[self.pc] = int(args[1],0)
                self.pc += 0x00000004

            if text_section_count : # .text의 line별 실행
                if len(args) > 1 :
                    if args[0] == "la" : # pseudo instruction # la $4, array2
                        ...
                        self.pc += 0x00000004 # instr.도 word이므로 32bit = 4B

                # line별 실행 내용
            elif args[0] == self.diretives[2] : # 최초 .text 등장
                self.pc = 0x00400000
    ...
    def args_to_binary(self, args) :
        if args[0] in self.instr_j_format :     # op jump_target
            ...
            op_hex = [9, 0xc, 4, 5, 0xf, 0x23, 0xd, 0xb, 0x2b, 0x20, 0x28]
    ...
    # type(hex(<int>))==str이고 self.labels.value==int이다.
...
if __name__ == "__main__":
    ...
    mips.save_files(sys.argv[1], binary = False);
